...
err <- sapply(1:200, function(x){rnorm(1,0,x)})
...
Linear models are about making predictions, so what these coefficients tell us is the predicted value of body mass for a specific bill length and species value. So, within a specific species, we expect that increasing bill length by one unit will increase body mass by approx. 90 units. We can kind of see that if we run the model only on one species at a time.
...
Let's try another one for bill length and bill depth
