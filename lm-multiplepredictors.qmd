...
mymodel3 <- lm(body_mass ~ bill_len, data=dplyr::filter(penguins2,species=="Chinstrap"))
mymodel4 <- lm(body_mass ~ bill_len, data=dplyr::filter(penguins2,species=="Gentoo"))
...
#legend('topleft',levels(penguins2$species),bty="n", pch=1, pt.lwd=2, col=palette())
abline(mymodel3, col=palette()[1], lwd=2)
abline(mymodel4, col=palette()[2], lwd=2)
abline(mymodel2, col="gray", lwd=2, lty=2)
abline(a=-267.623, b=102.298, col="black", lwd=2, lty=2)

legend("bottomright", c('Chinstrap','Gentoo','no species',"full model"), bty="n", col=c(palette()[2],palette()[1], "gray","black"), lwd=2, lty=c(1,1,2,2))
```
...
```{r}
model6 <- lm(body_mass~bill_len+species+bill_len*species,data=penguins2)
model6
Anova(model6)

```

In this case the statistical interaction is significant, but much less significant than the main effects. This is telling us that there is a slight difference in slopes between the two species, which we can see in @fig-interactions.

In particular, $b_3$ is the difference in slopes between the Chinstrap-only model and the Gentoo-only model. I was really surprised by this but the code below shows this.

```{r}
mymodel3$coefficients

mymodel4$coefficients

mymodel4$coefficients[[2]] - mymodel3$coefficients[[2]]
