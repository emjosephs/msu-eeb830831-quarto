...
### Linearity
We assume that we can model our response variable by adding up the values in the model equation. 

### Homoschedasticity
We assume that the variance of our residuals is the same across all of our data. If this is not true and the variance changes across residuals, then our data would be heteroschedastic. 
...
We assume that the observations of data point are independent from each other.

### Distribution 
For general (not generalized) linear models, we assume that our residuals are drawn from a single normal distribution.
