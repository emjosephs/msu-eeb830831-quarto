...
In this example, $\hat{Y}_i$ is estimated as the sum of the "intercept" ($a$), its value for the first explanatory variable ($x_{1,i}$) times the effect of this variable ($b_1$), its value for the second explanatory variable ($x_{2,i}$) times the effect of this variable, $b_2$, and so on for all included explanatory variables.
