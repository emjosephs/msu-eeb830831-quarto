...
```{r, echo=F, eval=F}
myN=100
insectCount <- runif(myN, min=0, max=20)
plantDamage <- insectCount*.3 + rnorm(myN,0,2)
data.frame(insectCount, plantDamage) |>
  ggplot() + aes(x=insectCount, y=plantDamage) +
  geom_point()+
  theme_classic()

mymodel <- lm(insectCount~plantDamage)
mymodel
summary(mymodel)
```
